...
export function downloadMarkdown(plan: SavedPlan) {
  const markdown = [
    ...
    '',
    '## Positioning',
    plan.positioning ?? plan.summary,
    '',
    section('Scope', plan.scope),
    section('MVP', plan.mvp),
    '## Success Metrics',
    ...(plan.success_metrics ?? []).map((item) => `- **${item.metric}** (${item.target}): ${item.why_it_matters}`),
    '',
    '## Product Decisions',
    ...(plan.product_decisions ?? []).map((item) => `- **${item.area}**: ${item.recommendation} Tradeoff: ${item.tradeoff}`),
    '',
    '## Features',
